...
#include "..\Commons\GRSWICHelper.h"
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		//创建RayTracing管线状态对象
		{
			...
			D3D12_SHADER_BYTECODE stLibDXIL
				= CD3DX12_SHADER_BYTECODE((void*)g_pRaytracing
					, ARRAYSIZE(g_pRaytracing));
			...
			auto objShaderConfig
				= objRaytracingPipeline.CreateSubobject<CD3D12_RAYTRACING_SHADER_CONFIG_SUBOBJECT>();
			...
			auto objLocalRootSignature
				= objRaytracingPipeline.CreateSubobject<CD3D12_LOCAL_ROOT_SIGNATURE_SUBOBJECT>();
			objLocalRootSignature->SetRootSignature(pIRSLocal.Get());
			// Define explicit shader association for the local root signature. 
			{
				auto objRootSignatureAssociation
					= objRaytracingPipeline.CreateSubobject<CD3D12_SUBOBJECT_TO_EXPORTS_ASSOCIATION_SUBOBJECT>();
			...
			auto objGlobalRootSignature
				= objRaytracingPipeline.CreateSubobject<CD3D12_GLOBAL_ROOT_SIGNATURE_SUBOBJECT>();
			objGlobalRootSignature->SetRootSignature(pIRSGlobal.Get());

			// Pipeline config
			// Defines the maximum TraceRay() recursion depth.
			auto objPipelineConfig
				= objRaytracingPipeline.CreateSubobject<CD3D12_RAYTRACING_PIPELINE_CONFIG_SUBOBJECT>();
